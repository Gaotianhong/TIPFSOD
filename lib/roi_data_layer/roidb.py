...
def prepare_roidb(imdb):
    ...
    for i in range(len(imdb.image_index)):
        roidb[i]['img_id'] = imdb.image_id_at(i)  # 图像在image_index列表中的序号
        roidb[i]['image'] = imdb.image_path_at(i)  # 当前训练图像的完整绝对路径
        if not (imdb.name.startswith('coco') or imdb.name.startswith('vg')):
    ...
    for idx, rdb in enumerate(roidb[:length]):
        ...
        for i in range(len(rdb['gt_classes'])):
            ...
            if shot == 0 and cls_id > base_num:
                # Choice 1: using images containing novel classes but treating them as background
                continue
        ...
        if len(boxes) > 0:
            new_roidb.append(
                {'boxes': np.array(boxes, dtype=np.uint16),  # 训练图像ground truth标注框信息
                 'gt_classes': np.array(gt_classes, dtype=np.int32),  # 训练图像ground truth类别信息
                 'gt_overlaps': gt_overlaps,  # 0-1向量，判断某个gt框是否属于gt_classes的类别
                 'flipped': rdb['flipped'],  # 表示训练图像是否需要水平翻转
                 'img_id': rdb['img_id'], 'image': rdb['image'],  # image为图像的绝对路径
                 'width': rdb['width'], 'height': rdb['height'],
                 'max_classes': np.array(max_classes),
                 'need_crop': rdb['need_crop'],
                 'max_overlaps': np.array(max_overlaps, dtype=np.float32)})
...
def combined_roidb(imdb_names, training=True):
    ...
    # ratio_list返回升序排列的宽高比值，ratio_index为升序后的图像编号
